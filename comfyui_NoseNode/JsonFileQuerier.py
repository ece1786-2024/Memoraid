...
class JsonFileQuerier:
    ...
    RETURN_NAMES = ("query_output", "update_output")
    ...
    def manipulate_json(self, input_str, folder_path):
        ...
        try:
            ...
            if action == "query":
                ...
                try:
                    result = self._parse_path(data, path)
                    debug_info.append(f"更新前数据: {result}")
                    ...
                    return {"ui": {"text": combined_output}, "result": ("", combined_output)}
                except Exception as e:
                    error_message = f"更新出错: {str(e)}"
                    return {"ui": {"text": error_message}, "result": ("", error_message)}
            else:
                # 返回空值
                return {"ui": {"text": "操作类型不支持，返回空值"}, "result": ("", "")}
